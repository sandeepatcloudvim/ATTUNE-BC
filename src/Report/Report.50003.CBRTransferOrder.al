report 50003 "CBR Transfer Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './CBRTransferOrder.rdl';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Transfer Line"; "Transfer Line")
                    {
                        ...
                        column(Lot_No1; LotNo[1])
                        {
                        }
                        column(Lot_No2; LotNo[2])
                        {
                        }
                        column(Lot_No3; LotNo[3])
                        {
                        }
                        column(Lot_No4; LotNo[4])
                        {
                        }
                        column(Lot_No5; LotNo[5])
                        {
                        }
                        column(LotQty1; LotQty[1])
                        {

                        }
                        column(LotQty2; LotQty[2])
                        {

                        }
                        column(LotQty3; LotQty[3])
                        {

                        }
                        column(LotQty4; LotQty[4])
                        {

                        }
                        column(LotQty5; LotQty[5])
                        {
                        ...
                        begin
                            ...
                            ReservEntry.Reset;
                            ReservEntry.SetRange("Source ID", "Transfer Line"."Document No.");
                            ReservEntry.SetRange("Source Ref. No.", "Transfer Line"."Line No.");
                            ReservEntry.SetRange("Item No.", "Transfer Line"."Item No.");
                            ReservEntry.SetFilter(Quantity, '>=%1', 1);
                            if ReservEntry.FindSet then
                                repeat
                                    i += 1;
                                    j += 1;
                                    LotNo[i] := ReservEntry."Lot No.";
                                    LotQty[j] := ReservEntry.Quantity;

                                until ReservEntry.Next = 0;

                            TracSpec.Reset;
                            TracSpec.SetRange("Source ID", "Transfer Line"."Document No.");
                            TracSpec.SetRange("Source Ref. No.", "Transfer Line"."Line No.");
                            TracSpec.SetRange("Item No.", "Transfer Line"."Item No.");
                            TracSpec.SetFilter("Quantity (Base)", '>=%1', 1);
                            if TracSpec.FindFirst then
                                repeat
                                    i += 1;
                                    j += 1;
                                    LotNo[i] := TracSpec."Lot No.";
                                    LotQty[j] := TracSpec."Quantity (Base)";
